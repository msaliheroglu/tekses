...
| Çok düğümlü dağıtım (2026-09-20) | Kue/müdahale yayınları düğümler arası **çekirdek NATS pub/sub** ile dağıtılır (`TEKSES_NATS_URL`; tekses.cast konusu, yayımcı dahil her düğüm alır) — tel zaten kayba dayanıklı (3× tekrar + run_id tekilleştirme), JetStream kalıcılığı gereksiz gecikme olur. **JetStream, kalıcı Run/telemetri akışına ayrıldı** (F2.6 ikinci yarı). `fire_at` kueyi alan düğümün saatinde hesaplanır; düğüm saatleri duvar-sabitli monoton olduğundan NTP'li makinelerde ~1 ms uyumludur. Yük ölçümü: tek 4-OCPU ücretsiz VM ≥40k istemci, yayılım 2 ms (docs/faz1-plan-ve-durum.md F2.4). |
