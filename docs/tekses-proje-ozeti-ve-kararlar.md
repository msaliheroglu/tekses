...
| Çok düğümlü dağıtım (2026-09-20) | Kue/müdahale yayınları düğümler arası **çekirdek NATS pub/sub** ile dağıtılır (`TEKSES_NATS_URL`; tekses.cast konusu, yayımcı dahil her düğüm alır) — tel zaten kayba dayanıklı (3× tekrar + run_id tekilleştirme), JetStream kalıcılığı gereksiz gecikme olur. `fire_at` kueyi alan düğümün saatinde hesaplanır; düğüm saatleri duvar-sabitli monoton olduğundan NTP'li makinelerde ~1 ms uyumludur. Yük ölçümü: tek 4-OCPU ücretsiz VM ≥40k istemci, yayılım 2 ms (docs/faz1-plan-ve-durum.md F2.4). |
| Telemetri (2026-09-20, F2.6 ikinci yarı) | Kalıcı Run izleri **JetStream'e değil** gateway→control-api HTTP iç ucuyla (`/internal/runs`, paylaşımlı `TEKSES_INTERNAL_TOKEN`; boşsa kapalı) Postgres'e yazılır — kontrol düzlemi hızı düşük, control-api NATS'siz kalır; kayıt yolu kue yolunu asla engellemez. Küme geneli katılımcı sayacı çekirdek NATS `tekses.presence` yayınıyla (5 sn rapor, 15 sn bayatlama, yaklaşık sayım). Saat kalitesi ısı haritası **sunucu tarafı ping-RTT** ile (keepalive ping'ine damga; sıfır tel değişikliği; RTT = senkron kalite VEKİLİ, ofset değil). Kova eşikleri sunucuda sabit: <10/<30/<100/≥100 ms. |
